...
"""
当 with 关键字修饰该对象的时候，会调用__enter__() 方法，它本质就是通过装饰模式包装了open()函数，真正的在__enter__()里执行，然后返回一个流。 外部通过 with File() as f 来接收(f就是接收返回的变量)，开流异常处理说明在enter内对open做了try处理。__exit__() 方法是走完该代码块执行，通常在该方法内做资源的释放（类比jdk1.8）
"""
```
